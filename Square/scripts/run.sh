...
for scale in 0.25 0.5 1 2; do
    ...
    srun --output="./outputs/heisenberg_${scale}_${bc}_${rate}.out" --ntasks=1 --cpus-per-task=$SLURM_CPUS_PER_TASK python -u enlargeSquare_scaling.py --seed 100 --experiment_name $exp_name	--bc $bc --units 256 --scale $scale --rate $rate&
done
wait